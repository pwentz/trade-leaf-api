...
dbSetup =
  let
    currentUser time =
      User "pat" "password" Nothing (Just $ toCoords 41.938132 (-87.642753)) time time
  in do
    ...
    user2Key <- runDb $ Sql.insert (User "fred" "password" Nothing (Just $ toCoords 41.858210 (-87.651700)) time time)
    ...
    user3Key <- runDb $ Sql.insert (User "bill" "password" Nothing (Just $ toCoords 42.858731 (-89.828149)) time time)
...
complexDbSetup =
  ...
  in do
    ...
    user1Key <- runDb $ Sql.insert (User "Otto" "password" Nothing (Just $ toCoords 41.858210 (-87.651700)) time time)
    ...
    user2Key <- runDb $ Sql.insert (User "Tim" "password" Nothing (Just $ toCoords 41.888730 (-87.687969)) time time)
    ...
    user3Key <- runDb $ Sql.insert (User "Theron" "password" Nothing (Just $ toCoords 41.680753 (-87.698157)) time time)
    ...
    user4Key <- runDb $ Sql.insert (User "Felicia" "password" Nothing (Just $ toCoords 41.734517 (-87.674043)) time time)
    ...
    user5Key <- runDb $ Sql.insert (User "Phil" "password" Nothing (Just $ toCoords 41.804575 (-87.671359)) time time)
